...
class OptionsFrame(ctk.CTkFrame):
    def __init__(self, controller, master, **kwargs):
        ...
        for i, (text, cmd) in enumerate(buttons):
            (ctk.CTkButton(self, text=text, command=cmd, **BUTTON_STYLE)
...
class MainFrame(ctk.CTkFrame):
    ...
    def populateTable(self,parent):
        ...
        for row in files.itertuples():
            ...
            client = ctk.CTkLabel(parent,text=client_name,corner_radius=0,width=75,justify="left",anchor="w")
            client.grid(row=row.Index+1,column=col,padx=(8,0),pady=5,sticky='w')
            col+=1

            if row.File_Status:
                status_label = ctk.CTkLabel(parent,text="Ready",fg_color="#388E3C",text_color="#E0E0E0",corner_radius=5,width=50,
                                            justify="left",anchor="w")
                status_label.grid(row=row.Index+1,column=col,pady=5,sticky='w')
            else:
                status_label = ctk.CTkLabel(parent,text="Not Ready",fg_color="#D97925",text_color="#E0E0E0",corner_radius=5,width=50,
                                            justify="left",anchor="w")
                status_label.grid(row=row.Index+1,column=col,pady=5,sticky='w')
            col+=1
